    ...
    <section aria-labelledby="book-details-heading" className="border-y border-stone-200 bg-[#eee7d9]">
      <div className="mx-auto grid max-w-6xl items-center gap-10 px-5 py-14 sm:px-8 sm:py-20 lg:grid-cols-[minmax(220px,300px)_1fr] lg:gap-16">
        <div className="flex justify-center">
          <img src={book.cover} alt={`${book.title} book cover`} width="360" height="520" className="max-h-[26rem] w-auto rounded-r-md shadow-xl" />
        </div>
        <div className="max-w-2xl">
          ...
          <dl className="mt-8 grid grid-cols-2 gap-5 border-y border-stone-300 py-5 text-sm">
            ...
            <div>
              ...
              <dd className="mt-1 font-semibold text-[#243e36]">
                {book.inStock ? 'In stock' : 'Out of stock'}
              </dd>
            </div>
          </dl>
          <div className="mt-7 flex flex-wrap items-center gap-5">
            <p className="text-2xl font-semibold text-[#243e36]">{formatNaira(book.price)}</p>
            <button
              type="button"
              onClick={() => addToCart(book)}
              title="Add this book to your cart"
              className="min-h-12 rounded-full bg-[#243e36] px-7 py-3.5 text-sm font-semibold text-white hover:bg-[#31584c]"
            >
              Add to Cart
            </button>
          </div>
