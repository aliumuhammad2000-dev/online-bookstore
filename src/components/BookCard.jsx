  ...
  return (
    <article className="flex h-full w-full max-w-xs flex-col overflow-hidden rounded-2xl border border-stone-200 bg-white">
      ...
      <div className="flex flex-1 flex-col p-5">
        <h3 className="font-serif text-xl font-bold leading-snug text-[#243e36]"><Link to={`/books/${id}`} className="underline decoration-transparent underline-offset-4 transition hover:decoration-amber-700">{title}</Link></h3>
        ...
        <button type="button" onClick={() => addToCart({ id, title, author, price, cover })} title="Add this book to your cart" className="mt-4 min-h-11 w-full rounded-full border border-stone-300 px-4 py-2.5 text-sm font-semibold text-[#243e36] hover:bg-stone-100">Add to Cart</button>
      </div>
    </article>
  )
}
